...
interface GPTPostComposerProps {
  onClose: () => void;
}

export default function GPTPostComposer({ onClose }: GPTPostComposerProps) {
  ...
  const generatePostMutation = useMutation({
    mutationFn: async (prompt: string) => {
      ...
      // const res = await apiRequest("POST", "/api/gpt/generate-post", { prompt });
  ...
  const publishPostMutation = useMutation({
    mutationFn: async (postData: { content: string, imageUrl?: string, gptName: string }) => {
  ...
  return (
    <Dialog open={true} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="text-lg font-medium">Compose with My GPT</DialogTitle>
        </DialogHeader>
        ...
        <div>
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <h4 className="text-base font-medium text-neutral-800">CreativeGPT</h4>
              <p className="text-sm text-neutral-500">Free Tier - General Creativity Assistant</p>
            </div>
          </div>
          
          <div className="mb-4">
            <label htmlFor="gptInputText" className="block text-sm font-medium text-neutral-700 mb-1">What's on your mind?</label>
            <Textarea 
              id="gptInputText"
              rows={3}
              value={input}
          ...
          {generatedImageUrl && (
            <div className="border-t border-neutral-200 pt-4 mb-4">
              ...
              <div className="relative">
                ...
                />
                <div className="absolute bottom-2 right-2 bg-secondary bg-opacity-90 text-white text-xs px-2 py-1 rounded">Generated by AI</div>
              </div>
